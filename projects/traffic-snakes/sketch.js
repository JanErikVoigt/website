...
function setup() {
  canvasElement = document.getElementById("canvas_container");

  cnv = createCanvas(800, 400);
  canvasElement.width = "800px"; //cnv.width;
  canvasElement.height = cnv.height;
  cnv.parent(canvasElement); // Replace the placeholder canvas with p5's
...
function draw() {
  ...
  for (let i = 0; i < lanes.length; i++) {
    if (snake_spawn_countdown[i] <= 0) {
      let new_speed =
        min_car_speed_ms +
        ((random(0, 1) + random(0, 1)) / 2) *
          (expected_car_speed_ms - min_car_speed_ms);
      ...
      if (amount_cars > 1) {
        ...
        for (let car_i = 1; car_i < lane.snakes[snake_i].cars.length; car_i++) {
          ...
          if (lane.snakes[snake_i].cars[car_i].target_speed < snakespeed) {
            ...
            let cars_infront_faster = subset(
              lane.snakes[snake_i].cars,
              0,
              car_i,
            );
            let cars_behind_slower = subset(
              lane.snakes[snake_i].cars,
              car_i,
              lane.snakes[snake_i].cars.length,
            );

            let snake_behind = new Snake(
              null,
              lanes[lane_i].snakes[snake_i].front -
                car_length * cars_infront_faster.length,
            ); //TODO also subtract speeddiff * timeDelta from front...
            snake_behind.cars = cars_behind_slower;

            snake_behind.front -=
              ((snakespeed - snake_behind.get_speed()) * deltaTime) / 1000;
  ...
  for (let lane_i = 1; lane_i < lanes.length; lane_i++) {
    for (let i = lanes[lane_i].snakes.length - 1; i >= 0; i--) {
      ...
      //subject range:
      let subject_range = [
        lanes[lane_i].snakes[i].front - car_length - 0.543,
        lanes[lane_i].snakes[i].front + 0.543,
      ]; //also dont, if there is not a carlength infront (??)
      //space on lane below
      if (!lanes[lane_i - 1].area_blocked(subject_range[0], subject_range[1])) {
        ...
        let success =
          lanes[lane_i - 1].insert_snake_at_right_position(ovetaker);
        if (!success) {
  ...
  for (let lane_i = 0; lane_i < lanes.length; lane_i++) {
    for (let i = lanes[lane_i].snakes.length - 1; i >= 0; i--) {
      if (i + 1 < lanes[lane_i].snakes.length) {
        if (
          lanes[lane_i].snakes[i].front >= lanes[lane_i].snakes[i + 1].rear()
        ) {
          //either join snake or switch lanes...
          let overtake = false;

          if (lane_i + 1 < lanes.length) {
            //another lane exists
            ...
            if (
              !lanes[lane_i + 1].area_blocked(
                subject.front - car_length,
                subject.front,
              )
            ) {
              // can overtake
              ...
              if (
                wanted_speed - snake_in_front_speed >=
                min_speed_diff_for_overtake
              ) {
          ...
          if (overtake) {
            ...
            //dont overtake and connect to end!
            lanes[lane_i].snakes[i + 1].cars = lanes[lane_i].snakes[
              i + 1
            ].cars.concat(lanes[lane_i].snakes[i].cars);
            //remove ith element
            lanes[lane_i].snakes = concat(
              subset(lanes[lane_i].snakes, 0, i),
              subset(lanes[lane_i].snakes, i + 1, lanes[lane_i].snakes.length),
            );
          }
        }
      }
  ...
  for (let lane_i = 0; lane_i < lanes.length - 1; lane_i++) {
    for (
      let snake_i = lanes[lane_i].snakes.length - 1;
      snake_i >= 0;
      snake_i--
    ) {
      ...
      if (amount_cars > 1) {
        ...
        for (let car_i = amount_cars - 1; car_i > 0; car_i--) {
          ...
          if (
            lanes[lane_i].snakes[snake_i].cars[car_i].target_speed >=
            snakespeed + min_speed_diff_for_overtake
          ) {
            //print("want overtake")
            //want overtake
            let car_area = [
              lanes[lane_i].snakes[snake_i].front - (car_i + 1) * car_length,
              ...
              if (car_i == amount_cars - 1 || car_i == 0) {
                if (car_i == amount_cars - 1) {
                  ...
                  if (car_i == 0) {
                    ...
                    let ovetaker =
                      lanes[lane_i].pop_first_car_from_snake(snake_i);
                    let success =
                      lanes[lane_i + 1].insert_snake_at_right_position(
                        ovetaker,
                      );
                    if (!success) {
                ...
                let new_snakes = lanes[lane_i].pop_car_from_snake(
                  snake_i,
                  car_i,
                );
                //insert behind at same lane
                let success1 = lanes[lane_i].insert_snake_at_right_position(
                  new_snakes[0],
                );

                //insert ovetaker at lane above
                let success2 = lanes[lane_i + 1].insert_snake_at_right_position(
                  new_snakes[1],
                );
                ...
                if (!success1 || !success2) {
                  /*print(
                    "FAILED INSERTION OF CAR ON DISCONNECT A MIDDLE CAR",
                    car_i,
                    success1,
                    success2,
                  );*/
...
class Lane {
  ...
  pop_front_car_of_snake(i) {
    if (this.snakes[i].cars.length == 1) {
      ...
      this.snakes[i].cars = subset(
        this.snakes[i].cars,
        1,
        this.snakes[i].cars.length,
      );
  ...
  pop_car_from_snake(snake_i, car_i) {
    if (car_i >= this.snakes[snake_i].cars.length - 1 || car_i <= 0) {
      ...
      //   "CAR ",
      //   car_i,
      //   " DOESNT EXIST; also 0 and len-1 are not allowed  len=",
      //   this.snakes[snake_i].cars.length,
      // );
      return null;
    }
    ...
    if (this.snakes[snake_i].cars.length == 1) {
      ...
      let cars_behind = subset(
        this.snakes[snake_i].cars,
        car_i + 1,
        this.snakes[snake_i].cars.length,
      );
      ...
      let snake_behind = new Snake(
        cars_behind[0],
        this.snakes[snake_i].front - car_length * (car_i + 1),
      );
      snake_behind.cars = cars_behind;
      let snake_overtaker = new Snake(
        middle_car,
        this.snakes[snake_i].front - car_length * car_i,
      );
...
class Car {
  ...
  draw(front_pos, y) {
    ...
    rect(
      (rear + 1) * x_meters_to_pxl,
      y - 7,
      (car_length - 2) * x_meters_to_pxl,
      14,
    );
  }
}
...
function spawn_addlane_buttons() {
  slider_min_cd = select("#mincd");
  slider_max_cd = select("#maxcd");

  slider_min_speed = select("#minspeed");
  slider_ev_speed = select("#expectedspeed");
}
...
function update_configs() {
  ...
  select("#mincd_value").html(min_car_cooldown);
  select("#maxcd_value").html(max_car_cooldown);
  ...
  select("#minspeed_value").html(str(round(min_car_speed_ms * 3.6)) + "km/h");
  select("#maxspeed_value").html(
    str(round((expected_car_speed_ms + diff1) * 3.6)) + "km/h",
  );
}
...
function draw_analytics() {
  ...
  while (
    !recent_despawns.length == 0 &&
    recent_despawns[0][0] < run_time - 10
  ) {
  ...
  textSize(16);
  text(
    "spawned:     " + str(round(recent_spawns.length / 10.0, 1)) + " cars/s",
    550,
    -180,
  );
  text(
    "despawned: " + str(round(despawned_sum / 10.0, 1)) + " cars/s",
    550,
    -162,
  );
